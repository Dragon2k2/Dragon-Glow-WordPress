 ...
 *
 * Single entry point for all Buy Now actions. Project uses WooCommerce
 * exclusively — mock products resolve to shadow WC products.
 *
 * Decision tree:
 ...
 *                                    else error: product unavailable
...
class DG_Checkout_Router {
	...
	private static function handle_by_slug( string $slug, int $quantity, string $size ): array {
		...
		if ( DG_Product_Factory::mock()->exists( $slug ) ) {
			...
			// ── Mock product found ───────────────────────────────────────────────
			// Project uses WooCommerce exclusively: resolve to shadow WC product,
			// add to WC cart, redirect to WC checkout.
			if ( dg_is_woocommerce_active() ) {
				$wc_handler = new DG_WooCommerce_Checkout_Handler();
				return $wc_handler->handle_mock_shadow( $slug, $quantity, $size );
			}

			// WC inactive — no mock checkout fallback; surface an explicit error.
			return array(
				'success' => false,
				'message' => __( 'This product is currently unavailable for purchase. Please contact support.', 'dragon-glow' ),
			);
		}
	...
	/**
	 * Attempt to dispatch a mock product via the WooCommerce shadow product path.
	 *
	 * Returns null if the slug is not a recognised mock product. Project uses
	 * WooCommerce exclusively — there is no separate mock-checkout page, so the
	 * mock-product path always resolves to a shadow WC product.
	 *
	 * @param string $slug
	 * @param int    $quantity
	 * @param string $size
	 * @return array|null Null if slug is not mock, otherwise the WC handler result.
	 */
