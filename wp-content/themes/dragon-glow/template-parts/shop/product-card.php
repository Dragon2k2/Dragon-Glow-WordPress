...
if ( ! empty( $tag_terms ) && ! is_wp_error( $tag_terms ) ) {
	$names = array_map( function( $t ) { return strtoupper( $t->name ); }, $tag_terms );
...
<div class="stagger-item group product-card-hover reveal-on-scroll active dg-product-card"
	...
	<div class="relative aspect-[3/4] overflow-hidden bg-surface-container-low mb-6 dg-product-image">
		<a href="<?php echo esc_url( $product_link ); ?>" aria-label="<?php echo esc_attr( $product_name ); ?>" class="block w-full h-full">
			<img alt="<?php echo esc_attr( $product_name ); ?>"
				 class="w-full h-full object-cover dg-product-img"
				 src="<?php echo esc_url( $image_url ); ?>" />
		</a>

		<?php if ( $badge_label ) : ?>
			<?php $pos_class = $is_dark ? 'absolute top-4 right-4 z-10' : 'absolute top-4 left-4 z-10'; ?>
		...
		<button class="dg-add-to-ritual dg-quick-add absolute z-10 inline-flex items-center justify-center gap-2"
	...
	<div class="text-center px-2">
		<h3 class="dg-product-name">
			<a href="<?php echo esc_url( $product_link ); ?>" class="dg-product-name-link">
